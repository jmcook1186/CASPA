...
time_tot = 15;
timestep = 1;
rand_event = randi(time_tot,1); % randomly choose a timestep where random event might occur
event_chance = 90; % percent chance that random event will occur sometime during run
...
for counter = 1:timestep:time_tot    
    ...
    if counter == rand_event % When the counter gets to the timestep when an event could occur...
        
        if rand < event_chance  % check whether random number 'rand' is < event_chance. If so, start an event
            EVENT = "SUDDEN EVENT AT TIMESTEP " + int2str(counter) % tell user that an event is occurring, and when
        
            for i = 2:1:gridx-1
                for j = 2:1:gridy-1
                    
                if grid(i,j) > 2
                    grid(i,j) = grid(i,j)-2; % for each cell where value > 2, subtract 2 (reduce algal biomass) AMEND EVENT DETAILS HERE
                else
                    grid(i,j) = grid(i,j); % if cell value < 2, leave it as it is
                end
            end
            end
        end
